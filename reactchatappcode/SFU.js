...
const deleteUser = (socketID, roomID, userID) => {
...
io.sockets.on("connection", (socket) => {
  ...
  socket.on("senderOffer", async (data) => {
    try {
      ...
      {
        io.to(data.senderSocketID).emit("loginFailed", {
          errormessage: "You can not connect to the same room twice",
        });
      }
      else
      {
        users[data.userID] = 1;*/
        userToRoom[data.senderSocketID] = data.userID;
    ...
    {
      socket.userID = data.userID;
      isconnected[data.userID] = false;
      users[data.userID] = data.userID;
      io.to(data.senderSocketID).emit("Getlocal", {
        id: "asd"
      });
    }
  });
  ...
  socket.on("disconnect", () => {
    try {
      if(isconnected[socket.userID] == false)
      {
        let roomID = socketToRoom[socket.id];
        let userID = userToRoom[socket.id];
        deleteUser(socket.id, roomID, userID);
      ...
      {
        isconnected[users[socket.userID]] = false
      }
    } catch (error) {
      console.log(error);
    }
